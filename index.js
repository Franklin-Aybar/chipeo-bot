...
const app = express();
app.get('/', (req, res) => res.send('✅ Ciphero Online'));
...
const client = new Client({
    intents: [
        ...
        GatewayIntentBits.MessageContent, // requerido por DisTube v5
    ]
});
...
const distube = new DisTube(client, {
    ...
    plugins: [
        new YouTubePlugin({
            // Si YouTube bloquea en Render, agrega cookies via variable de entorno:
            // cookies: process.env.YOUTUBE_COOKIES ? JSON.parse(process.env.YOUTUBE_COOKIES) : undefined
        })
    ]
});
...
client.on('interactionCreate', async (interaction) => {
    ...
    // ── Comandos que requieren cola activa ──
